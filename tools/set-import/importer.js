...
function validSet(
	source, dex, format, pokemon, name, set
) {
...
function toPokemonSet(dex, format, pokemon, set) {
...
function getLevel(format, level = 0) {
 ...
 function getStatisticsURL(index, format) {
...
async function importThirdPartySets(
	...
	for (const formatid in data.files) {
		...
		const json = JSON5.parse(match[1]) ;
...
function fixThirdParty(dex, pokemon, set) {
