...
let BattleMovedex = {
	...
	protect: {
		inherit: true,
		desc: "The user is protected from attacks on the turn it's used.",
		priority: 2,
		effect: {
			duration: 1,
			onStart(target) {
	...
	triattack: {
		inherit: true,
		desc: "This move selects burn, freeze, or paralysis at random, and has a 30% chance to inflict the target with that status. If the target is frozen and burn was selected, it thaws out.",
		onHit(target, source, move) {
	...
	gigadrain: { //New moves not listed prior begin here.
		inherit: true,
		pp: 10,
	},
	hiddenpower: {
	...
	furycutter: {
		inherit: true,
		desc: "Has a 20% chance to double Base Power next turn.",
		basePower: 25,
		type: "Bug",
		accuracy: 100,
		effect: {
	...
	blizzard: {
		inherit: true,
		shortDesc: "30% chance to freeze the target.", //Yep.
		accuracy: 90,
		basePower: 120,
		secondary: {
			chance: 30,
	...
	"return": {
		...
		basePowerCallback(pokemon) {
			return Math.floor((pokemon.happiness * 10) / 25) || 50;
		},
	},
	frustration: {
		inherit: true,
	...
	rapidspin: {
		...
		onAfterMove(pokemon) {
			pokemon.addVolatile('partialtrappinglock');
		},
	},
	irontail: {
		inherit: true,
